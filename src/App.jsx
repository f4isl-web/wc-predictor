...
export default function App() {
      ...
      fbListen("matches_v2", v=>{
        if(v){
          ...
          const merged = arr.map(m => {
            ...
            if(!def) return m;
            // Knockout matches (id >= 73) preserve their teams (set manually in admin);
            // group matches sync teams from code so fixture corrections apply.
  ...
  const refreshKnockout=async()=>{
    ...
    for(const def of DEFAULT_MATCHES){
      ...
      const merged = { ...def, result: existing.result || def.result };
      await fbSet(`matches_v2/${def.id}`, merged);
      n++;
    }
    showToast(`Knockout refreshed (${n} matches)`);
  };
