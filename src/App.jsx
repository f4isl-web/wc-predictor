...
// ─── Matches ───────────────────────────────────────────────────────────────────
// Bracket progression: which two matches feed each knockout match, and whether
// the winner (W) or loser (L) advances. Used to auto-fill teams as results are entered.
// Format: targetMatchId: { home:[sourceId, "W"|"L"], away:[sourceId, "W"|"L"] }
const BRACKET_MAP = {
  // Round of 16
  89:{home:[74,"W"],away:[77,"W"]}, 90:{home:[73,"W"],away:[75,"W"]},
  91:{home:[76,"W"],away:[78,"W"]}, 92:{home:[79,"W"],away:[80,"W"]},
  93:{home:[83,"W"],away:[84,"W"]}, 94:{home:[81,"W"],away:[82,"W"]},
  95:{home:[86,"W"],away:[88,"W"]}, 96:{home:[85,"W"],away:[87,"W"]},
  // Quarter-finals
  97:{home:[89,"W"],away:[90,"W"]}, 98:{home:[93,"W"],away:[94,"W"]},
  99:{home:[91,"W"],away:[92,"W"]}, 100:{home:[95,"W"],away:[96,"W"]},
  // Semi-finals
  101:{home:[97,"W"],away:[98,"W"]}, 102:{home:[99,"W"],away:[100,"W"]},
  // Third place (losers) and Final (winners)
  103:{home:[101,"L"],away:[102,"L"]}, 104:{home:[101,"W"],away:[102,"W"]},
};

// Pure bracket propagation usable outside the component.
function propagateBracketPure(arr, fbSetFn){
  const byId = Object.fromEntries(arr.map(m=>[m.id,m]));
  const decided = (m, want) => {
    const r=m?.result; if(!r||r.home==null||r.home===""||r.away==null||r.away==="") return null;
    const h=parseInt(r.home), a=parseInt(r.away); if(isNaN(h)||isNaN(a)) return null;
    let win=null, lose=null;
    if(h>a){win=m.home;lose=m.away;} else if(a>h){win=m.away;lose=m.home;}
    else { if(r.penWinner==="home"){win=m.home;lose=m.away;} else if(r.penWinner==="away"){win=m.away;lose=m.home;} }
    if(!win) return null;
    return want==="W"?win:lose;
  };
  let changed=false;
  const out = arr.map(m=>{
    const map = BRACKET_MAP[m.id]; if(!map) return m;
    const newHome = decided(byId[map.home[0]], map.home[1]);
    const newAway = decided(byId[map.away[0]], map.away[1]);
    let nm=m;
    if(newHome && newHome!==m.home){ nm={...nm,home:newHome}; changed=true; fbSetFn&&fbSetFn(`matches_v3/${m.id}/home`,newHome); }
    if(newAway && newAway!==m.away){ nm={...nm,away:newAway}; changed=true; fbSetFn&&fbSetFn(`matches_v3/${m.id}/away`,newAway); }
    return nm;
  });
  return changed ? propagateBracketPure(out, fbSetFn) : out;
}
...
const DEFAULT_MATCHES = [
  ...
  {id:75,phase:"Round of 32",home:"Netherlands 🇳🇱",away:"Morocco 🇲🇦",date:"Jun 30",kickoffTime:"2026-06-30T04:00+03:00"},
  {id:76,phase:"Round of 32",home:"Brazil 🇧🇷",away:"Japan 🇯🇵",date:"Jun 29",kickoffTime:"2026-06-29T20:00+03:00"},
  ...
  {id:79,phase:"Round of 32",home:"Mexico 🇲🇽",away:"Best 3rd (C/E/F/H/I)",date:"Jul 1",kickoffTime:"2026-07-01T02:00+03:00"},
  {id:80,phase:"Round of 32",home:"England 🏴󠁧󠁢󠁥󠁮󠁧󠁿",away:"Best 3rd (E/H/I/J/K)",date:"Jul 2",kickoffTime:"2026-07-02T01:00+03:00"},
  {id:81,phase:"Round of 32",home:"USA 🇺🇸",away:"Bosnia & Herz. 🇧🇦",date:"Jul 2",kickoffTime:"2026-07-02T03:00+03:00"},
  {id:82,phase:"Round of 32",home:"Winner Group G",away:"Best 3rd (A/E/H/I/J)",date:"Jul 2",kickoffTime:"2026-07-02T20:00+03:00"},
  {id:83,phase:"Round of 32",home:"Runner-up K",away:"Runner-up L",date:"Jul 3",kickoffTime:"2026-07-03T01:00+03:00"},
  {id:84,phase:"Round of 32",home:"Spain 🇪🇸",away:"Runner-up J",date:"Jul 3",kickoffTime:"2026-07-03T03:00+03:00"},
  {id:85,phase:"Round of 32",home:"Switzerland 🇨🇭",away:"Best 3rd (E/F/G/I/J)",date:"Jul 3",kickoffTime:"2026-07-03T20:00+03:00"},
  {id:86,phase:"Round of 32",home:"Argentina 🇦🇷",away:"Cape Verde 🇨🇻",date:"Jul 4",kickoffTime:"2026-07-04T01:00+03:00"},
  {id:87,phase:"Round of 32",home:"Colombia 🇨🇴",away:"Best 3rd (D/E/I/J/L)",date:"Jul 4",kickoffTime:"2026-07-04T03:00+03:00"},
  {id:88,phase:"Round of 32",home:"Australia 🇦🇺",away:"Belgium 🇧🇪",date:"Jul 4",kickoffTime:"2026-07-04T20:00+03:00"},
...
export default function App() {
      ...
      fbListen("matches_v3", v=>{
        if(v){
          ...
          const merged = arr.map(m => {
            ...
            if(!def) return m;
            // All knockout matches (id >= 73) preserve their teams — R32 set in code/admin,
            // R16+ auto-filled by bracket. Fresh v3 seed already has correct R32 teams.
            const preserveTeams = m.id >= 73;
          ...
          const propagated = propagateBracketPure(merged, fbSet);
          if(needsUpdate) fbSet("matches_v3", Object.fromEntries(propagated.map(m=>[m.id,m])));
          setMatches(propagated);
        } else {
          const seed = Object.fromEntries(DEFAULT_MATCHES.map(m=>[m.id,m]));
          fbSet("matches_v3", seed);
  ...
  const setMatchResult=(id,field,val)=>{
    fbSet(`matches_v3/${id}/result/${field}`,val);
    setMatches(prev=>{
      const updated = prev.map(m=>m.id===id?{...m,result:{...m.result,[field]:val}}:m);
      return propagateBracketPure(updated, fbSet);
    });
  };
  const setMatchKickoff=(id,val)=>{
    ...
    fbSet(`matches_v3/${id}/kickoffTime`,withTz);
