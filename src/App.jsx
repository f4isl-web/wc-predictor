...
// ─── Blackout window 11:00–13:00 Riyadh ───────────────────────────────────────
...
// ─── Scoring: +1 outcome, +3 exact, +1 per scorer ─────────────────────────────
...
async function fetchLiveScores(matches, setMatches) {
  try {
    const res = await fetch(`${API_BASE_PROXY}/fixtures?league=${WC_LEAGUE}&season=${WC_SEASON}&timezone=Asia/Riyadh`, { headers: { "x-apisports-key": API_KEY } });
    if (!res.ok) return;
    const data = await res.json();
    const fixtures = (data.response || []).filter(f => ["FT","AET","PEN","1H","2H","HT","ET","P","LIVE"].includes(f.fixture?.status?.short));
    ...
    setMatches(prev => {
      ...
      const next = prev.map(m => {
        ...
        const fix = fixtures.find(f => {
          const fH = (f.teams?.home?.name||"").toLowerCase(), fA = (f.teams?.away?.name||"").toLowerCase();
        ...
        const homeScorers = events.filter(e => e.team?.id===fix.teams?.home?.id && e.type==="Goal" && e.detail!=="Own Goal").map(e => e.player?.name||"");
        const awayScorers = events.filter(e => e.team?.id===fix.teams?.away?.id && e.type==="Goal" && e.detail!=="Own Goal").map(e => e.player?.name||"");
        const newResult = { home: String(goals.home), away: String(goals.away), homeScorers, awayScorers, autoSynced: true };
...
function Pill({ children, active, onClick }) {
  return <button onClick={onClick} style={{background:active?"#e8c000":"rgba(255,255,255,.05)",color:active?"#0a0d18":"#7a8aaa",border:`1px solid ${active?"#e8c000":"rgba(255,255,255,.08)"}`,borderRadius:20,padding:"5px 14px",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:12,transition:"all .15s"}}>{children}</button>;
}
...
function PinPad({ value, onChange, label }) {
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:10,marginBottom:16,justifyContent:"center"}}>{[0,1,2,3].map(i=><div key={i} style={{width:16,height:16,borderRadius:"50%",background:value.length>i?"#e8c000":"rgba(255,255,255,.15)",transition:"background .15s"}}/>)}</div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,maxWidth:200,margin:"0 auto"}}>
      {digits.map((d,i)=><button key={i} onClick={()=>{if(d===null)return;if(d==="⌫")onChange(value.slice(0,-1));else if(value.length<4)onChange(value+d);}} style={{background:d===null?"transparent":"rgba(255,255,255,.07)",border:d===null?"none":"1px solid rgba(255,255,255,.1)",borderRadius:10,padding:"14px 0",cursor:d===null?"default":"pointer",color:d==="⌫"?"#e8c000":"#f0f0f0",fontFamily:"'DM Sans',sans-serif",fontSize:d==="⌫"?18:20,fontWeight:600}}>{d}</button>)}
    </div>
  </div>;
}
...
function ScorerPicker({ value, onChange, squad, placeholder, disabled }) {
  ...
  return <select value={value||""} onChange={e=>onChange(e.target.value)} disabled={disabled||!hasPlayers} style={{background:disabled||!hasPlayers?"rgba(255,255,255,.03)":"rgba(100,140,255,.08)",border:"1px solid rgba(100,140,255,.2)",borderRadius:8,padding:"6px 10px",color:value?"#c0d0e8":"#4a5a7a",fontSize:12,fontFamily:"'DM Sans',sans-serif",cursor:disabled||!hasPlayers?"default":"pointer",width:"100%"}}>
    <option value="">{hasPlayers?(placeholder||"— pick player —"):"No squad"}</option>
    {isGrouped ? groups.map(g=><optgroup key={g.label} label={g.label}>{g.players.map(p=><option key={p} value={p}>{p}</option>)}</optgroup>) : flatPlayers.map(p=><option key={p} value={p}>{p}</option>)}
  </select>;
}
...
export default function App() {
  ...
  const [squads]                      = useState(DEFAULT_SQUADS);
  ...
  useEffect(() => {
    // Load initial data then set up live listeners
    const unsubUsers    = fbListen("users",    v => { if(v) setUsers(v); });
    const unsubMatches  = fbListen("matches",  v => { if(v) { const arr = Object.values(v); arr.sort((a,b)=>a.id-b.id); setMatches(arr); } else { setMatches(DEFAULT_MATCHES); fbSet("matches", Object.fromEntries(DEFAULT_MATCHES.map(m=>[m.id,m]))); } });
    const unsubPreds    = fbListen("predictions", v => { if(v) setPredictions(v); });
    setLoaded(true);
    return () => { unsubUsers(); unsubMatches(); unsubPreds(); };
  }, []);

  // ── API live score polling ────────────────────────────────────────────────────
  useEffect(() => {
    if (!loaded) return;
    const sync = async () => {
      if (isBlackoutWindow()) return;
      setApiSyncing(true);
      await fetchLiveScores(matches, newMatches => {
        // Write updated matches to Firebase
        setMatches(prev => {
  ...
  const handleAuth = async () => {
    ...
    if (authMode === "register") {
      const existingUsers = await fbGet("users");
      if (existingUsers?.[name]) return setAuthErr("Username already taken");
      const isAdmin = !existingUsers || Object.keys(existingUsers).length === 0 || isSuperAdmin(name);
      await fbSet(`users/${name}`, { pin: authPin, isAdmin, superAdmin: isSuperAdmin(name) });
  ...
  const setPred = (matchId, field, val) => {
    const path = `predictions/${session}/${matchId}/${field}`;
    fbSet(path, val);
    setPredictions(prev => ({ ...prev, [session]: { ...prev[session], [matchId]: { ...prev[session]?.[matchId], [field]: val } } }));
  };

  const setMatchResult = (matchId, field, val) => {
    fbSet(`matches/${matchId}/result/${field}`, val);
    setMatches(prev => prev.map(m => m.id === matchId ? { ...m, result: { ...m.result, [field]: val } } : m));
  };
  const setMatchKickoff = (matchId, val) => {
    fbSet(`matches/${matchId}/kickoffTime`, val);
    setMatches(prev => prev.map(m => m.id === matchId ? { ...m, kickoffTime: val } : m));
  };
    ...
    <div style={{minHeight:"100vh",background:"#080c18",color:"#f0f0f0",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{position:"relative",overflow:"hidden",background:"linear-gradient(160deg,#0f1428,#0a0e1c)",borderBottom:"1px solid rgba(232,192,0,.15)"}}>
        ...
        <div style={{position:"relative",padding:"20px 20px 14px",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:34,letterSpacing:4,color:"#e8c000",lineHeight:1}}>⚽ WC PREDICTOR 2026</div>
            <div style={{fontSize:11,color:"#4a5a7a",marginTop:4,display:"flex",alignItems:"center",gap:8,fontWeight:600,flexWrap:"wrap"}}>
              ...
              <span style={{fontSize:10,color:"#2a3a5a"}}>{apiSyncing?"⏳ syncing scores…":isBlackoutWindow()?"🚫 API paused 11–13":lastApiSync?`⚽ ${lastApiSync.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}`:"⚽ pending"}</span>
            </div>
          </div>
          {session?(
      ...
      <div style={{padding:"22px 16px",maxWidth:760,margin:"0 auto"}}>
        ...
        {page==="scoreboard"&&<div className="fi">
          ...
          <div style={{fontSize:12,color:"#4a5a7a",marginBottom:18,background:"rgba(255,255,255,.03)",borderRadius:10,padding:"10px 14px",lineHeight:1.9}}>
            ...
            <span style={{color:"#6699ff",fontWeight:700}}>+1 per scorer</span> guessed correctly
          </div>
          ...
          {leaderboard.map((p,i)=>{
            ...
            return <div key={p.name} style={{background:i===0?"linear-gradient(135deg,rgba(232,192,0,.1),rgba(232,192,0,.03))":"rgba(255,255,255,.03)",border:`1px solid ${i===0?"rgba(232,192,0,.25)":"rgba(255,255,255,.06)"}`,borderRadius:12,padding:"14px 18px",marginBottom:8,display:"flex",alignItems:"center",gap:14,animation:`fadeIn .3s ${i*.06}s ease both`}}>
        ...
        {page==="matches"&&<div className="fi">
          ...
          {visibleMatches.map(m=>{
            ...
            return <div key={m.id} style={{background:"rgba(255,255,255,.025)",border:`1px solid ${hasResult?"rgba(68,204,136,.15)":"rgba(255,255,255,.05)"}`,borderRadius:10,padding:"12px 14px",marginBottom:6}}>
        ...
        {page==="players"&&<div className="fi">
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:24}}>
            {Object.keys(users).map(u=><button key={u} onClick={()=>setSelectedPlayer(selectedPlayer===u?null:u)} style={{background:selectedPlayer===u?"#e8c000":"rgba(255,255,255,.05)",color:selectedPlayer===u?"#080c18":"#c0d0e8",border:`1px solid ${selectedPlayer===u?"#e8c000":"rgba(255,255,255,.08)"}`,borderRadius:20,padding:"7px 18px",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:13}}>
              {u} <span style={{color:selectedPlayer===u?"#080c18":"#e8c000",fontWeight:700,marginLeft:4}}>{leaderboard.find(l=>l.name===u)?.total||0}pts</span>
            </button>)}
          </div>
          {selectedPlayer&&(()=>{
            ...
            return <div>
              <div style={{background:"linear-gradient(135deg,rgba(232,192,0,.1),rgba(232,192,0,.03))",border:"1px solid rgba(232,192,0,.2)",borderRadius:12,padding:"14px 18px",marginBottom:20,display:"flex",gap:20,flexWrap:"wrap",alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
                  {[["#e8c000",lb.outcome||0,"OUTCOME"],["#44cc88",lb.exact||0,"EXACT"],["#6699ff",lb.scorerPts||0,"SCORER PTS"],["#7a8aaa",lb.played||0,"PLAYED"]].map(([c,v,l])=><div key={l} style={{textAlign:"center"}}><div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:24,color:c}}>{v}</div><div style={{fontSize:10,color:"#4a5a7a"}}>{l}</div></div>)}
                </div>
              </div>
              ...
              {visibleMatches.map(m=>{
                ...
                const res=m.result, hasResult=res?.home!=null&&res?.home!=="";
                ...
                return <div key={m.id} style={{background:"rgba(255,255,255,.025)",border:"1px solid rgba(255,255,255,.04)",borderRadius:10,padding:"11px 14px",marginBottom:6}}>
                  <div style={{display:"flex",alignItems:"center",gap:10}}>
                    <div style={{minWidth:70}}><div style={{fontSize:9,color:"#e8c000",fontWeight:700}}>{m.phase}</div><div style={{fontSize:11,color:"#3a4a6a"}}>{m.date}</div></div>
                    <div style={{flex:1,fontWeight:600,fontSize:13}}>{m.home}</div>
                    <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:18,color:"#e8c000",minWidth:50,textAlign:"center"}}>{pred?.homeScore!=null&&pred?.awayScore!=null?`${pred.homeScore}–${pred.awayScore}`:<span style={{color:"#2a3a5a",fontSize:12}}>—</span>}</div>
                    <div style={{fontSize:10,color:"#3a4a6a",padding:"0 4px"}}>→</div>
                    <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:18,color:hasResult?"#44cc88":"#2a3a5a",minWidth:50,textAlign:"center"}}>{hasResult?`${res.home}–${res.away}`:"TBD"}</div>
                    <div style={{flex:1,fontWeight:600,fontSize:13,textAlign:"right"}}>{m.away}</div>
                    <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:26,color:ptsColor,minWidth:36,textAlign:"right"}}>{pts!==null?`+${pts}`:"—"}</div>
                  </div>
                  {(allPred.length>0||allReal.length>0)&&<div style={{marginTop:8,paddingTop:8,borderTop:"1px solid rgba(255,255,255,.05)",display:"flex",gap:16,fontSize:11,flexWrap:"wrap"}}>
        ...
        {page==="predict"&&session&&<div className="fi">
          ...
          <div style={{fontSize:12,color:"#4a5a7a",marginBottom:16}}>Predictions lock 2h before kickoff. +1 per correct scorer, any order.</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:18}}>{phases.map(ph=><Pill key={ph} active={phaseFilter===ph} onClick={()=>setPhaseFilter(ph)}>{ph}</Pill>)}</div>
          {visibleMatches.map(m=>{
            ...
            const homeScorers=pred.homeScorers||[], awayScorers=pred.awayScorers||[];
            return <div key={m.id} style={{background:"rgba(255,255,255,.03)",border:`1px solid ${locked&&!hasResult?"rgba(255,100,0,.2)":hasResult?"rgba(255,255,255,.05)":"rgba(255,255,255,.07)"}`,borderRadius:12,padding:"14px",marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                <div>
                  <span style={{fontSize:10,color:"#e8c000",fontWeight:700,letterSpacing:1}}>{m.phase}</span>
                  <div style={{fontSize:11,color:"#4a5a7a"}}>{m.date}{m.kickoffTime&&!hasResult&&<span style={{marginLeft:8,color:locked?"#ff6666":countdown?"#44cc88":"#7a8aaa"}}>{locked?"🔒 locked":countdown?`⏳ ${countdown}`:"✅ open"}</span>}</div>
                </div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                <div style={{flex:1,fontWeight:600,fontSize:13}}>{m.home}</div>
                <input type="number" min="0" max="20" value={pred.homeScore??""} onChange={e=>{setPred(m.id,"homeScore",e.target.value);setPred(m.id,"homeScorers",[]);}} disabled={locked} style={{width:44,textAlign:"center",background:locked?"rgba(255,255,255,.03)":"rgba(232,192,0,.08)",border:`1px solid ${locked?"rgba(255,255,255,.06)":"rgba(232,192,0,.3)"}`,borderRadius:8,padding:"7px 2px",color:"#e8c000",fontSize:18,fontWeight:700}}/>
                <span style={{color:"#3a4a6a",fontWeight:700}}>–</span>
                <input type="number" min="0" max="20" value={pred.awayScore??""} onChange={e=>{setPred(m.id,"awayScore",e.target.value);setPred(m.id,"awayScorers",[]);}} disabled={locked} style={{width:44,textAlign:"center",background:locked?"rgba(255,255,255,.03)":"rgba(232,192,0,.08)",border:`1px solid ${locked?"rgba(255,255,255,.06)":"rgba(232,192,0,.3)"}`,borderRadius:8,padding:"7px 2px",color:"#e8c000",fontSize:18,fontWeight:700}}/>
                <div style={{flex:1,fontWeight:600,fontSize:13,textAlign:"right"}}>{m.away}</div>
              </div>
              {(homeSlots>0||awaySlots>0)&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div>
                  <div style={{fontSize:10,color:"#4a5a7a",fontWeight:700,marginBottom:5}}>⚽ {m.home} ({homeSlots})</div>
                  {Array.from({length:homeSlots}).map((_,si)=><div key={si} style={{marginBottom:4}}><ScorerPicker value={homeScorers[si]||""} onChange={v=>{const arr=[...homeScorers];arr[si]=v;setPred(m.id,"homeScorers",arr);}} squad={homeSquad} placeholder={`Goal ${si+1}…`} disabled={locked}/></div>)}
                </div>
                <div>
                  <div style={{fontSize:10,color:"#4a5a7a",fontWeight:700,marginBottom:5}}>⚽ {m.away} ({awaySlots})</div>
                  {Array.from({length:awaySlots}).map((_,si)=><div key={si} style={{marginBottom:4}}><ScorerPicker value={awayScorers[si]||""} onChange={v=>{const arr=[...awayScorers];arr[si]=v;setPred(m.id,"awayScorers",arr);}} squad={awaySquad} placeholder={`Goal ${si+1}…`} disabled={locked}/></div>)}
                </div>
              </div>}
              {hasResult&&<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid rgba(255,255,255,.05)",fontSize:12,color:"#4a5a7a"}}>
        ...
        {page==="auth"&&!session&&<div className="fi" style={{maxWidth:320,margin:"0 auto",paddingTop:20}}>
          ...
          <div style={{display:"flex",marginBottom:24,background:"rgba(255,255,255,.04)",borderRadius:10,padding:3}}>
            {["login","register"].map(m=><button key={m} onClick={()=>{setAuthMode(m);setAuthErr("");setAuthPin("");}} style={{flex:1,background:authMode===m?"#e8c000":"transparent",color:authMode===m?"#080c18":"#4a5a7a",border:"none",borderRadius:8,padding:"8px 0",cursor:"pointer",fontWeight:700,fontSize:13,transition:"all .15s"}}>{m==="login"?"Sign In":"Register"}</button>)}
          </div>
          <div style={{marginBottom:20}}>
            <label style={{fontSize:12,color:"#7a8aaa",display:"block",marginBottom:6,fontWeight:600}}>USERNAME</label>
            <input value={authName} onChange={e=>setAuthName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleAuth()} placeholder="Your name…" style={{width:"100%",background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:10,padding:"11px 14px",color:"#f0f0f0",fontSize:15,fontFamily:"'DM Sans',sans-serif"}}/>
          </div>
        ...
        {page==="admin"&&isAdmin&&<div className="fi">
          <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:26,letterSpacing:3,color:"#e8c000",marginBottom:20,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
            ADMIN PANEL
            <button onClick={async()=>{setApiSyncing(true);await fetchLiveScores(matches,newM=>{setMatches(prev=>{const u=typeof newM==="function"?newM(prev):newM;fbSet("matches",Object.fromEntries(u.map(m=>[m.id,m])));return u;});});setLastApiSync(new Date());setApiSyncing(false);showToast("Scores synced!");}} style={{background:"rgba(68,204,136,.15)",border:"1px solid rgba(68,204,136,.3)",color:"#44cc88",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:700,fontSize:12}}>
          ...
          <div style={{marginBottom:32}}>
            ...
            {visibleMatches.map(m=>{
              ...
              const homeSquad=squads[m.home]||{}, awaySquad=squads[m.away]||{};
              return <div key={m.id} style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.07)",borderRadius:12,padding:"13px 14px",marginBottom:8}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                  <div style={{fontSize:10,color:"#e8c000",fontWeight:700,minWidth:80}}>{m.phase}<br/><span style={{color:"#4a5a7a"}}>{m.date}</span>{m.result?.autoSynced&&<span style={{display:"block",color:"#44cc88",fontSize:9}}>⚽ auto</span>}</div>
                  <div style={{flex:1,fontWeight:600,fontSize:13}}>{m.home}</div>
                ...
                {(homeSlots>0||awaySlots>0)&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:4}}>
                  <div>
                    <div style={{fontSize:10,color:"#44cc88",fontWeight:700,marginBottom:4}}>⚽ {m.home} ({homeSlots})</div>
                    {Array.from({length:homeSlots}).map((_,si)=><div key={si} style={{marginBottom:4}}>
                      <ScorerPicker value={(res.homeScorers||[])[si]||""} onChange={v=>{const arr=[...(res.homeScorers||[])];arr[si]=v;setMatchResult(m.id,"homeScorers",arr);}} squad={homeSquad} placeholder={`Goal ${si+1}…`}/>
                    </div>)}
                  </div>
                  <div>
                    <div style={{fontSize:10,color:"#44cc88",fontWeight:700,marginBottom:4}}>⚽ {m.away} ({awaySlots})</div>
                    {Array.from({length:awaySlots}).map((_,si)=><div key={si} style={{marginBottom:4}}>
                      <ScorerPicker value={(res.awayScorers||[])[si]||""} onChange={v=>{const arr=[...(res.awayScorers||[])];arr[si]=v;setMatchResult(m.id,"awayScorers",arr);}} squad={awaySquad} placeholder={`Goal ${si+1}…`}/>
                    </div>)}
                  </div>
                </div>}
              </div>;
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:8}}>
              {Object.entries(users).map(([name,data])=><div key={name} style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.07)",borderRadius:10,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <div>
                ...
                {name!==session&&!isSuperAdmin(name)&&<button onClick={async()=>{await fbSet(`users/${name}`,null);showToast(`${name} removed`);}} style={{background:"none",border:"none",color:"#ff4466",cursor:"pointer",fontSize:18}}>×</button>}
              </div>)}
            </div>
          </div>
        </div>}
