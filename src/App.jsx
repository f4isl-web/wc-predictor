    ...
    const uns = [
      ...
      fbListen("matches", v=>{
        if(v){
          ...
          // Merge kickoffTime from DEFAULT_MATCHES if missing in Firebase
          let needsUpdate = false;
          const merged = arr.map(m => {
            const def = DEFAULT_MATCHES.find(d=>d.id===m.id);
            if(def?.kickoffTime && !m.kickoffTime) { needsUpdate=true; return {...m, kickoffTime:def.kickoffTime}; }
            return m;
          });
          if(needsUpdate) fbSet("matches", Object.fromEntries(merged.map(m=>[m.id,m])));
          setMatches(merged);
        } else {
          fbSet("matches", Object.fromEntries(DEFAULT_MATCHES.map(m=>[m.id,m])));
        }
      }),
              ...
              return <div key={m.id} style={{background:T.card,border:`1px solid ${locked&&!hasRes?"rgba(255,100,0,.2)":T.border}`,borderRadius:12,padding:"14px",marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <div>
                    ...
                    <div style={{fontSize:11,color:T.dim}}>{m.date}{m.kickoffTime&&!hasRes&&<span style={{marginLeft:8,color:locked?"#ff6666":deadline?"#44cc88":"#7a8aaa"}}>{locked?"🔒 closed":deadline?`⏳ ${deadline} to predict`:"✅ open"}</span>}</div>
                  </div>
