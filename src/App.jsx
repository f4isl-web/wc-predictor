...
export default function App() {
  ...
  const setPred=(matchId,field,val)=>{
    ...
    const myLeagues = isSuperAdmin(session) ? [activeLeague] : (users[session]?.leagues || [activeLeague]);
    const targetLeagues = (matchId >= MIRROR_FROM_MATCH && myLeagues.length > 1) ? myLeagues : [activeLeague];
    setPredictions(prev=>{
      const next = {...prev};
  ...
  const syncMultiLeaguePicks=async()=>{
    ...
    const multi = Object.keys(users).filter(u=>!isSuperAdmin(u) && (users[u]?.leagues||[]).length>1);
    for(const u of multi){
      const myLeagues = users[u].leagues;
      let touched=false;
      for(const m of matches){
        if(m.id < MIRROR_FROM_MATCH) continue;
        // find a league where this player already has a pick for this match
        let source=null;
        for(const lg of myLeagues){
          const p = predictions[lg]?.[u]?.[m.id];
          if(p && (p.homeScore!=null || p.awayScore!=null || (p.homeScorers||[]).length || (p.awayScorers||[]).length)){ source=p; break; }
        }
        if(!source) continue;
        // write that pick to ALL the player's leagues
        for(const lg of myLeagues){
    ...
    showToast(`Synced ${playersAffected} players · ${copied} picks copied`);
  };
