...
async function fetchLiveScores(matches, onUpdated, onStatus) {
  ...
  // A match is only "played" if its status says so. Unplayed games often report 0/0 or null
  // with a Not Started / scheduled status, so we must check strStatus — not just the score.
  const finishedStatuses = ["match finished","ft","aet","pen","after extra time","penalties","finished"];
  const liveStatuses = ["1h","2h","ht","et","first half","second half","half time","live","in progress","playing"];
  const played = events.filter(e => {
    const st = (e.strStatus||"").trim().toLowerCase();
    const isFinished = finishedStatuses.some(s=>st.includes(s));
    const isLive = liveStatuses.some(s=>st.includes(s));
    // Must be finished or live AND have actual numeric scores
  ...
  const updated=matches.map(m=>{
    ...
    if(!ev) return m;
    // Extra safety: never update a match whose kickoff hasn't happened yet
    if(m.kickoffTime && new Date(m.kickoffTime).getTime() > Date.now()) return m;
