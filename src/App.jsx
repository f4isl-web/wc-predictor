...
function calcPoints(pred, result) {
  ...
  // Scorer points: each real goal can only be matched once.
  // A player predicted N times only scores up to the number of real goals they actually scored.
  const realPool=(result.homeScorers||[]).concat(result.awayScorers||[]).map(s=>s.toLowerCase().trim()).filter(Boolean);
  const predList=(pred?.homeScorers||[]).concat(pred?.awayScorers||[]).map(s=>s.toLowerCase().trim()).filter(Boolean);
  const remaining=[...realPool]; // consumable copy
  predList.forEach(p=>{
    if(!p) return;
    const idx=remaining.findIndex(r=>r===p||r.includes(p)||p.includes(r));
    if(idx!==-1){ pts+=1; remaining.splice(idx,1); } // consume that goal so it can't be matched again
  });
  return pts;
}
...
// ─── Live Scores via TheSportsDB (free key 123, World Cup league 4429) ─────────
// Maps app team names (with flags) to TheSportsDB names
function normTeam(s){
  return s.replace(/\p{Emoji}/gu,"").replace(/[^a-zA-Z ]/g,"").trim().toLowerCase()
...
function teamsMatch(appName, dbName){
  const a=normTeam(appName), b=dbName.toLowerCase().trim();
  return a===b || a.includes(b) || b.includes(a);
}

async function fetchLiveScores(matches, onUpdated, onStatus) {
  const target = `https://www.thesportsdb.com/api/v1/json/123/eventsseason.php?id=4429&s=2026`;
  const attempts = [
    target, // TheSportsDB allows CORS directly
  ...
  for (const url of attempts) {
    try {
      ...
      if (json && json.events) { data = json; break; }
      lastErr = "no events";
    } catch(e) { lastErr = e.message; }
  }
  if (!data) { onStatus && onStatus(`Sync failed: ${lastErr}`); return; }

  const events = data.events || [];
  // Only events that have a final score entered
  const played = events.filter(e => e.intHomeScore!=null && e.intHomeScore!=="" && e.intAwayScore!=null && e.intAwayScore!=="");
  if(!played.length){ onStatus && onStatus("No finished matches yet"); return; }
...
export default function App() {
    ...
    const uns = [
      ...
      fbListen("matches_v2", v=>{
        if(v){
          ...
          // Always ensure kickoffTime from code is present
          let needsUpdate = false;
          const merged = arr.map(m => {
            const def = DEFAULT_MATCHES.find(d=>d.id===m.id);
            if(def?.kickoffTime && m.kickoffTime!==def.kickoffTime) { needsUpdate=true; return {...m, kickoffTime:def.kickoffTime}; }
            return m;
          });
    ...
    matches.forEach(m=>{
      ...
      if(pts!==null){
        ...
        if(res&&pred){
          ...
          const rem=[...rs];
          ps.forEach(p=>{ if(!p) return; const i=rem.findIndex(r=>r===p||r.includes(p)||p.includes(r)); if(i!==-1){ scorerPts++; rem.splice(i,1); } });
        }
      }
    });
  ...
  const setPredFor=(leagueId,uname,matchId,field,val)=>{
    ...
    fbSet(`predictions/${leagueId}/${uname}/${matchId}/${field}`,val);
    setPredictions(prev=>({...prev,[leagueId]:{...prev[leagueId],[uname]:{...prev[leagueId]?.[uname],[matchId]:{...prev[leagueId]?.[uname]?.[matchId],[field]:val}}}}));
  };
