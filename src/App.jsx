...
export default function App() {
  ...
  // Get a player's prediction for a match in a given league.
  // For matches from France-Senegal onward, if this league has no pick,
  // fall back to the same player's pick in ANY of their other leagues (auto-mirror on read).
  const getPred = (leagueId, uname, matchId) => {
    ...
    if(matchId >= MIRROR_FROM_MATCH){
      const userLeagues = isSuperAdmin(uname) ? Object.keys(leagues) : (users[uname]?.leagues || []);
    ...
    return here; // may be undefined/empty
  };
